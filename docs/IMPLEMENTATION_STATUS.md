...
The first isolated follow-up applies only two independent behaviors: the TargetPage deletion key is forced to read-only `2310` when the form loads, changes, or submits; and the app auto-dismisses only the exact `configured target host` Cookie retry alert. Form-position switching and list loading are intentionally unchanged.
