...
final class CompactPageModeServiceTests: XCTestCase {
    ...
    func testScriptIncludesRequestedFocusModeBehavior() {
        ...
        XCTAssertTrue(script.contains("node.id !== \"reszb\""))
        XCTAssertTrue(script.contains("fixedDeleteKey = \"2310\""))
        XCTAssertTrue(script.contains("enforceDeleteKey"))
        XCTAssertTrue(script.contains("configureFormPositionToggle"))
        XCTAssertTrue(script.contains("フォームを下へ"))
        XCTAssertTrue(script.contains("フォームを上へ"))
