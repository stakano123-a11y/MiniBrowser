...
Assert-Contains $focusModeFile 'fixedDeleteKey\s*=\s*"2310"' 'fixed TargetPage delete key'
Assert-Contains $focusModeFile 'configureFormPositionToggle' 'restored TargetPage form position switch'
...
Assert-Contains $listServiceFile 'replyCount\s*<\s*1_000' 'full TargetPage thread filter'
...
Assert-Contains $webViewFile 'WebDialogPolicy\.shouldAutoDismissAlert' 'narrow TargetPage cookie notice suppression'
